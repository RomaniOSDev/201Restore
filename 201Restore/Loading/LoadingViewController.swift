...
final class LoadingViewController: UIViewController {
    ...
    private func performConfigRequest() {
        ...
        ConfigManager.shared.requestConfig { [weak self] result in
            ...
            case .success(let response):
                if response.statusCode == 200, let urlString = response.url, let url = URL(string: urlString) {
                    self.removeConversionObserver()
                    self.transitionToWebView(url: url)
                } else if response.statusCode != 200 {
                    self.handleNon200ConfigResponse()
                } else {
                    self.handleSuccessfulConfigWithoutURL()
                }
            case .failure:
                self.handleNon200ConfigResponse()
            }
    ...
    /// HTTP ≠ 200 (или сеть): если на диске уже есть url — открываем его.
    private func handleNon200ConfigResponse() {
        didStartConfigRequest = false
        ...
        retryConfigOrGoNative()
    }

    /// HTTP 200, но оффера нет — в натив, сохранённый url не используем.
    private func handleSuccessfulConfigWithoutURL() {
        didStartConfigRequest = false
        retryConfigOrGoNative()
    }

    private func retryConfigOrGoNative() {
